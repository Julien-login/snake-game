...
let snake = [{ x: 9 * box, y: 10 * box, color: snakeHeadColor }];
let direction;
let isPaused = false;
let score = 0;
...
function draw() {
  ...
  const newHead = { x: snakeX, y: snakeY, color: snakeHeadColor };
  ...
  if (checkCollision(newHead, snake)) {
    ...
    score = 0;
    snake = [{ x: 9 * box, y: 10 * box, color: snakeHeadColor }];
    direction = undefined;
    food = createFood();
  ...
  if (snakeX === food.x && snakeY === food.y) {
    score++;
    snake[snake.length - 1].color = foodColor;
    food = createFood();
    foodColor = food.color;
  } else {
    snake.pop();
